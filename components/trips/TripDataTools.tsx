...
export function TripDataTools({ trips, onChange }: TripDataToolsProps) {
  ...
  const sizeLabel = serializedBytes > 1024 * 1024
    ? `${(serializedBytes / 1024 / 1024).toFixed(1)} MB`
    : `${Math.max(1, Math.round(serializedBytes / 1024))} KB`;
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 mb-6">
      <button type="button" disabled={trips.length === 0} onClick={() => downloadText("travelguide-trips.json", serialized, "application/json")} className="min-h-[40px] px-3 py-2 rounded-xl border border-surface-200 bg-white text-sm text-surface-700 hover:bg-surface-50 disabled:opacity-40">
        导出备份 JSON
      </button>
      ...
      <span className="text-xs text-surface-400">{trips.length} 份 · 约 {sizeLabel}</span>
